...
```{r plot horses, fig.width=10, fig.height=3, fig.cap= "Figure 1: Horse population historical data by province"}
...
max_horses <- horse_pop %>% 
  ...
  summarize(Maximum = max(Value),
            Minimum = min(Value))
knitr::kable(max_horses, caption = "Table 1: Horse population by province in Canada") |> 
  kableExtra::kable_styling(full_width = FALSE)
```

Below we zoom in and look at the province of `r params$province`:

```{r plot province, fig.width=8, fig.height=5, fig.cap= "Figure 2: Horse population historical data per province", fig.align='center'}
